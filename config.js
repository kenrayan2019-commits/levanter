...
module.exports = {
  ...
  STATUS_REPLY_EMOJI: process.env.STATUS_REPLY_EMOJI || '❤️',
  SEND_READ: toBool(process.env.SEND_READ || 'true'),
  AUTO_STATUS_SAVE: toBool(process.env.AUTO_STATUS_SAVE || 'false'),
  ...
  DELETE_TYPE: (process.env.DELETE_TYPE || 'pvt').trim(),
  LIST_TYPE: (process.env.LIST_TYPE || 'text').trim(),
  BING_COOKIE: (process.env.BING_COOKIE || '').trim(),
  GEMINI_API_KEY: (process.env.GEMINI_API_KEY || '').trim(),
  GEMINI_MODEL: (process.env.GEMINI_MODEL || 'gemini-2.0-flash').trim(),
